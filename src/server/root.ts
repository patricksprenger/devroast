...
export const roastSchema = z.object({
	...
	verdict: z
		...
		.max(300)
		.describe(
			"Um comentário extremamente curto (máximo de 5 a 6 palavras) sobre o código. Ex: 'Precisa de ajuda séria', 'Código impecável'.",
		),
	roastQuote: z
		.string()
		.describe(
			"Uma frase sarcástica e ácida humilhando amigavelmente as escolhas do desenvolvedor.",
		),
	improvedCode: z
		.string()
		...
		.array(
			z.object({
				...
				title: z
					.string()
					.describe(
						"Um título curto e técnico para o erro (ex: 'Global Variable Pollution').",
					),
				description: z
					.string()
					.describe(
						"A explicação detalhada do porquê isso é um erro e como afeta o código.",
					),
				severity: z
					.enum(["info", "warning", "critical", "good"])
					.describe(
						"O nível de gravidade do erro. Escolha estritamente entre info, warning, critical ou good.",
					),
				improvedCode: z
					.string()
					.describe(
						"Um pequeno trecho de código mostrando apenas como corrigir esta linha específica.",
					),
			}),
		)
		.describe(
			"Uma lista detalhada de todos os problemas encontrados no código.",
		),
});
